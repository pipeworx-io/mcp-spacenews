...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Spacenews');
}
...
async function getArticles(limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/articles/?${params}`);
  if (!res.ok) throw await httpError(res, 'Spaceflight News API error');
...
async function getBlogs(limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/blogs/?${params}`);
  if (!res.ok) throw await httpError(res, 'Spaceflight News API error');
